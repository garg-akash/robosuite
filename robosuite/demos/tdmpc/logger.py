...
class VideoRecorder:
	...
	def __init__(self, root_dir, wandb, render_size=384, fps=15):
		...
		self.save_dir = make_dir(root_dir / 'eval_video')
		self._wandb = wandb
	...
	def save(self, step):
		if self.enabled:
			...
			video_filename = os.path.join(self.save_dir, f"video_{step}.mp4")
			with imageio.get_writer(video_filename, fps=self.fps) as writer:
				for frame in frames:
					writer.append_data(frame)  # Write each frame to the video
